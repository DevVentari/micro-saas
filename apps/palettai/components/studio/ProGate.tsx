...
export function ProGate({ isPro, children, featureName }: ProGateProps) {
  ...
  return (
    <div className="relative">
      <div aria-hidden="true" className="pointer-events-none select-none blur-sm opacity-60">{children}</div>
      <div aria-label={`${featureName} is a Pro feature`} className="absolute inset-0 flex items-center justify-center">
        <div className="rounded-2xl border border-border bg-background/95 shadow-xl p-6 text-center max-w-xs mx-4">
          ...
          <Button asChild className="mt-4 w-full gap-2">
            <a href="/pricing"><Zap aria-hidden="true" className="size-4" />Upgrade to Pro — $5/mo</a>
          </Button>
        </div>
      </div>
    </div>
